...
<html lang=en-GB>
 ...
 <title>Libary Form Submission</title>

<body>
Thank you for adding a game to the library.
<?php
...
function validate_code()
{
	...
	if($_SERVER["REQUEST_METHOD"]=="POST")
	{
		...
		{
			$availableErr="Availabilty is required";
			
		} 
		else
		{
			$available=test_input($_POST["available"]);
			if(!preg_match("/^[a-z A-Z 0-9]*$/",$available))
			{
				$availableErr="Please use only letters, numbers and spaces";
				
			}
		}
		if(empty($_POST["notes"]))
		{
			$notesErr="Type of Event is required";
		}
		else
		{
			...
			{
				$notesErr="Please enter event type";
			}
		}
	}
	
	$valid=false;
	if($memberIDErr=="" && $firstNameErr=="" && $boardgameErr==""  && $availableErr=="" && $notesErr=="")
	{
		$valid=true;
	}
	return $valid;
...
function insert_form($conn, $tempMemberID, $tempFirstName, $tempBoardgame, $tempAvailable, $tempNotes)
{

 $sql ="INSERT INTO `library` (`MemberID`, `FirstName`, `Boardgame`, `Available`, `Notes`)
VALUES ('$tempMemberID', '$tempFirstName', '$tempBoardgame', '$tempAvailable', '$tempNotes')";
...
if($valid==true){
$result=insert_form($conn, $tempMemberID, $tempFirstName, $tempBoardgame, $tempAvailable, $tempNotes);
}
